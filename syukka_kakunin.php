...
foreach($_POST['books'] as $books_ => $book_id){
	/*
	 * ⑫POSTの「stock」について⑩の変数の値を使用して値を取り出す。
	 * 半角数字以外の文字が設定されていないかを「is_numeric」関数を使用して確認する。
	 * 半角数字以外の文字が入っていた場合はif文の中に入る。
	 */
	$stock = $_POST['stock'][$books_];
	if (!is_numeric($stock)) {
	...
	$total = $book['stock'] - $_POST['stock'][$books_];
...
if(!empty($_POST['add']) == 'ok'){
	...
	foreach($_POST['books'] as $books_ => $book_id){
		...
		$update_book = updateByid($book_id, $pdo, $total);
		//㉙ ㉔で宣言した変数をインクリメントで値を1増やす。
		// ++$book_count;
	}
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td><?php echo	$_POST['stock'][$books_];?></td>
					</tr>
					<input type="hidden" name="books[]" value="<?php echo $book['id'];?>">
					<input type="hidden" name="stock[]" value='<?php echo $_POST['stock'][$books_];?>'>
					<?php
						//㊵ ㉜で宣言した変数をインクリメントで値を1増やす。
						// ++$book_num;
					}
